"""
Extract results from already-trained model size experiment.
...
MODEL_CONFIGS = [
    {'name': 'nano', 'short': 'yolo11n', 'params': '2.6M'},
    {'name': 'small', 'short': 'yolo11s', 'params': '9.4M'},
    {'name': 'medium', 'short': 'yolo11m', 'params': '20.1M'},
    {'name': 'large', 'short': 'yolo11l', 'params': '25.3M'},
]
EPOCHS = 50

def extract_metrics_from_run(model_config):
    """Extract metrics from an existing training run."""
    run_path = os.path.join(RESULTS_BASE, model_config['short'])
    csv_path = os.path.join(run_path, "results.csv")
    
    if not os.path.exists(csv_path):
        print(f"❌ No results found for {model_config['name']} at {csv_path}")
        return None
...
def generate_markdown_report(log):
    ...
    md = f"""# YOLO Model Size Experiment Report
...
| Augmentation | {log['config']['augmentation']} |
...
"""
    ...
    for r in log['results']:
        if 'error' not in r:
            md += f"| {r['model']} | {r['parameters']} | {r['epochs']['completed']}/{r['epochs']['total']} | "
            md += f"{r['epochs']['best_epoch']} | "
            md += f"{'Yes' if r['epochs']['early_stopped'] else 'No'} | "
            md += f"{r['training_time_formatted']} |\n"
    
    md += "\n## Results Summary\n\n"
    md += "| Model | Precision | Recall | F1 Score | mAP50 | mAP50-95 | Train Loss | Val Loss |\n"
    md += "|-------|-----------|--------|----------|-------|----------|------------|----------|\n"
    ...
    md += "| Model | Best Epoch | Best mAP50 | Best mAP50-95 | Best Precision | Best Recall |\n"
    md += "|-------|------------|------------|---------------|----------------|-------------|\n"
    
    for r in log['results']:
        if 'error' not in r:
            md += f"| {r['model']} | {r['best_metrics']['epoch']} | "
    ...
    if valid_results:
        ...
        md += f"\n## Conclusion\n\n"
        md += f"**Best Model:** {best['model']}\n\n"
        md += f"### Best Model Performance\n"
        md += f"- Parameters: {best['parameters']}\n"
        ...
        md += f"This model size provides the best balance between accuracy, speed, and resource usage.\n"
    
    md += f"\n## Model Weights\n\n"
    md += f"All trained models are saved in `{RESULTS_BASE}`:\n"
    for r in log['results']:
        if 'error' not in r:
            model_short = r['model'].replace('YOLO11-', 'yolo11')
            md += f"- `{model_short}/weights/best.pt` (best at epoch {r['best_metrics']['epoch']})\n"
            md += f"- `{model_short}/weights/last.pt` (final at epoch {r['epochs']['completed']})\n"
...
def main():
    ...
    for model_config in MODEL_CONFIGS:
        print(f"Processing YOLO11-{model_config['name']}...")
        result = extract_metrics_from_run(model_config)
        if result:
    ...
    output_dir = "./model_size_experiments"
    ...
    print(f"{'='*80}")
    print(f"{'Model':<15} {'Params':<10} {'Precision':<12} {'Recall':<12} {'F1':<12} {'mAP50-95':<12}")
    print("-" * 80)
    for r in experiment_log['results']:
        print(f"{r['model']:<15} {r['parameters']:<10} {r['final_metrics']['precision']:<12.4f} "
